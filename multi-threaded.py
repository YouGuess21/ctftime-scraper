...
def scrape_writeup(url, retries=3):
    headers = {
    ...
    for attempt in range(retries):
        try:
            ...
            if e.response.status_code == 503:
                print(f'503 error on {url}, attempt {attempt + 1} of {retries}')
    ...
    raise requests.exceptions.HTTPError(f'Failed to scrape {url} after {retries} attempts')

def main():
    base_url = 'https://ctftime.org/writeup/'
    urls = [base_url + str(i).zfill(5) for i in range(1, 39318)]  # Adjust the range as needed
